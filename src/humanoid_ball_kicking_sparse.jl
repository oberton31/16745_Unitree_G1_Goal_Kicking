...
#         # 0.5*(x - x_g)'*Q2*(x-x_g)
#         return 0.5*(foot_tip_pos - desired_foot_pos)'*Q1*(foot_tip_pos - desired_foot_pos) + 0.5 * z[ui]'*R*z[ui]
#     end
...
#     # primal bounds
#     z_l, z_u = fill(-1200.0, param.nz), fill(1200.0, param.nz)
...
function optimize_trajectory_sparse(nx, nu, dt, N, x_eq, u_eq, equilib_foot_pos, kick_foot_pos, model, z0, lower_joint_limits, upper_joint_limits)
    Q1 = diagm(1e0*ones(3))
    Q2 = diagm(1e0*ones(nx))
    R = diagm(1e-2*ones(nu))
    Qf = diagm(1e2*ones(3))
